...
final class LegalDocumentContent
{
    ...
    {
        return array_map(fn (string $text) => ['type' => 'paragraph', 'parts' => [['type' => 'text', 'text' => $text]]], $texts);
    }

    private static function h(string $text): array
    {
        return ['type' => 'subheading', 'parts' => [['type' => 'text', 'text' => $text]]];
    }

    private static function b(string ...$items): array
    {
    ...
    {
        return ['type' => 'paragraph', 'parts' => $parts];
    }

    private static function t(string $text): array
    {
        return ['type' => 'text', 'text' => $text];
    }

    private static function i(string $target): array
    {
        return ['type' => 'internal_reference', 'target' => $target];
    }

    private static function c(string $field): array
    {
    ...
    private static function privacy(string $key): array
    {
        return match ($key) {
            ...
            'mandatory_optional' => self::p('I dati contrassegnati come necessari o indispensabili per una richiesta, una prenotazione, un account o una prestazione devono essere forniti per poter erogare il servizio. In mancanza, potremmo non riuscire a completare l’operazione o a dare seguito alla richiesta.', 'Il conferimento per newsletter, preferenze non necessarie e finalità basate sul consenso è facoltativo. Il rifiuto o la revoca non impediscono l’uso dei servizi essenziali né la ricezione delle comunicazioni di servizio.'),
            'processing_security' => self::p('I dati sono trattati con strumenti digitali e, quando necessario, cartacei, secondo principi di liceità, correttezza, trasparenza, minimizzazione e limitazione delle finalità. Remedic adotta misure tecniche e organizzative proporzionate ai rischi per proteggere riservatezza, integrità e disponibilità dei dati. Nessun sistema può tuttavia garantire una sicurezza assoluta: proteggi le tue credenziali e informa subito il centro in caso di uso sospetto dell’account.'),
    ...
    private static function cookie(string $key): array
    {
        return match ($key) {
            ...
            'manage_preferences' => self::p('Puoi cambiare in qualsiasi momento le categorie opzionali tramite il controllo “Gestisci preferenze cookie” presente nel footer. La revoca vale per il futuro e non rende illecito un trattamento svolto prima della revoca.', 'Puoi inoltre cancellare cookie e dati locali dalle impostazioni del browser, bloccarne l’uso o impostare avvisi. La disattivazione degli strumenti necessari può impedire il corretto funzionamento di preferenze, autenticazione o servizi richiesti.'),
            'policy_updates' => [self::parts(self::t('Aggiorniamo questa pagina quando cambiano gli strumenti utilizzati, i fornitori o le regole applicabili. La data indicata in alto identifica la versione corrente. Per domande sul trattamento dei dati o sugli strumenti di tracciamento consulta la '), self::i('privacy'), self::t(' e i recapiti in essa indicati.'))],
            default => [],
        };
    }

    private static function terms(string $key): array
    {
        return match ($key) {
            'scope' => self::p('Questi termini disciplinano l’accesso e l’uso del sito Remedic, delle funzioni di contatto e prenotazione, dell’area riservata e degli altri servizi digitali messi a disposizione dal centro. Utilizzando una funzione specifica accetti anche le condizioni e le informative mostrate nel relativo percorso.'),
            'health_information_emergencies' => self::p('I contenuti del sito hanno finalità informative e orientative. Non costituiscono diagnosi, prescrizione, consulto medico individuale né sostituiscono la valutazione di un professionista sanitario.', 'Il sito e i canali digitali Remedic non sono servizi di emergenza. In caso di urgenza, pericolo immediato o sintomi gravi, contatta il numero unico di emergenza o rivolgiti senza ritardo ai servizi sanitari competenti. Non inviare richieste urgenti tramite moduli, e-mail, newsletter o chat.'),
            'professionals_services' => self::p('Profili, aree di attività, disponibilità e contenuti clinici descrivono i servizi offerti e possono essere aggiornati. Il rapporto sanitario nasce con il professionista o la struttura che eroga la prestazione secondo le condizioni comunicate al paziente. Le indicazioni presenti sul sito non garantiscono l’idoneità di una prestazione per il singolo caso clinico.'),
            'requests_bookings' => self::p('Il percorso di prenotazione permette di richiedere un appuntamento o avviare un contatto con il centro. Una richiesta si considera accettata soltanto dopo la conferma resa attraverso i canali indicati. Disponibilità, professionista, sede e orario possono richiedere una verifica organizzativa.', 'Se non puoi presentarti, avvisa il centro appena possibile attraverso i recapiti ufficiali. Eventuali regole specifiche su modifica o cancellazione vengono comunicate durante la prenotazione o nella conferma. Il sito non prevede pagamenti online: gli aspetti amministrativi della prestazione sono gestiti secondo le indicazioni fornite dal centro.'),
            'account_reserved_area' => [...self::p('L’area riservata consente di accedere a funzioni personali collegate a prenotazioni, appuntamenti, comunicazioni, referti o documenti. Devi fornire informazioni corrette, mantenere aggiornati i recapiti e custodire le credenziali con cura.'), self::b('non condividere password o codici di accesso con persone non autorizzate;', 'non utilizzare l’account di altri e non tentare di superare le misure di sicurezza;', 'informa subito il centro se sospetti un accesso improprio o la perdita delle credenziali;', 'esci dall’account quando utilizzi dispositivi condivisi.'), ...self::p('Remedic può sospendere l’accesso quando necessario per sicurezza, manutenzione, violazioni di questi termini o tutela dei dati, limitatamente al tempo necessario a gestire la situazione.')],
            'medical_reports_documents' => self::p('I referti e gli altri documenti resi disponibili digitalmente sono destinati esclusivamente all’interessato o a chi è legittimato ad accedervi. L’utente deve verificarne l’intestazione, conservarli in modo sicuro e seguire le istruzioni ricevute dal professionista.', 'La disponibilità online non sostituisce il confronto clinico quando indicato. In caso di documento mancante, illeggibile o apparentemente riferito a un’altra persona, non divulgarlo e contatta subito il centro.'),
            'service_communications_newsletter' => self::p('Le comunicazioni necessarie a gestire una richiesta, un appuntamento, un account o un documento sono parte del servizio richiesto. È responsabilità dell’utente mantenere aggiornati i recapiti e controllare le comunicazioni ricevute.', 'La newsletter è facoltativa e distinta dalle comunicazioni di servizio. Puoi revocare l’iscrizione senza perdere l’accesso alle funzioni essenziali. Non utilizzare la newsletter per inviare richieste cliniche o urgenti.'),
            'proper_use' => [...self::p('Quando utilizzi il sito e i servizi digitali, ti impegni a:'), self::b('fornire dati veritieri, pertinenti e aggiornati;', 'rispettare i diritti, la riservatezza e la sicurezza delle altre persone;', 'non introdurre codice dannoso, automatismi abusivi o carichi che possano compromettere il servizio;', 'non copiare, alterare o riutilizzare contenuti in violazione della legge o dei diritti di Remedic e di terzi;', 'non usare il sito per finalità illecite, ingannevoli o estranee ai servizi offerti.')],
            'service_availability_changes' => self::p('Remedic cura l’aggiornamento e la continuità dei servizi, ma non garantisce che siano sempre disponibili o privi di errori. Manutenzioni, aggiornamenti, guasti, eventi esterni o esigenze di sicurezza possono causare sospensioni. Quando possibile, le funzioni vengono ripristinate e le informazioni rilevanti vengono comunicate con modalità adeguate.'),
            'intellectual_property' => self::p('Testi, marchi, immagini, grafica, struttura e software del sito sono protetti dalle norme applicabili e appartengono a Remedic o ai rispettivi titolari. È consentita la consultazione per uso personale. Ogni altro uso, riproduzione o diffusione richiede una base giuridica o l’autorizzazione del titolare dei diritti.'),
            'third_party_links' => self::p('Il sito può rimandare a mappe, social network, WhatsApp o altri servizi esterni. Questi servizi sono gestiti dai rispettivi fornitori e soggetti alle loro condizioni e informative. Remedic non controlla disponibilità, contenuti o trattamento dei dati sui siti di terzi e invita l’utente a verificarne le regole prima dell’uso.'),
